...
router.post(`/addingredientinrecipe`, async (req, res) => {
    ...
    if(data.rows.length === 0) {
        res.json({message: "records already exists"});
    } else {

        try {
            const result = await db.query(`INSERT INTO ingredientsinrecipe (recipeid, ingredientid) SELECT a.id, b.id FROM recipe a JOIN ingredient b ON a. recipeName = $1 AND b.ingredientname = $2;`, [recipename, ingredientname]);
            console.log(result.rowCount);
